...
public class SQLCache {
    ...
    public void level_1() {
        ...
        NovelDto n1 = session.selectOne(NovelMapper.class.getName() + ".queryByID", 1);
        log.info("一 次查询---end");

        log.info("二 次查询---start");
        NovelDto n2 = session.selectOne(NovelMapper.class.getName() + ".queryByID", 0);
        log.info("二 次查询---end");

        // 从日志中可以看出这次并没有发起sql查询，一级缓存
        log.info("三 次查询---start");
        NovelDto n3 = session.selectOne(NovelMapper.class.getName() + ".queryByID", 1);
        log.info("三 次查询---end");
    ...
    public void level_2() {
        ...
        NovelDto n1 = session_1.selectOne(NovelMapper.class.getName() + ".queryByID", 1);
        log.info("session_1 次查询---end");
        session_1.commit();
        session_1.close();

        log.info("session_2 次查询---start");
        NovelDto n2 = session_2.selectOne(NovelMapper.class.getName() + ".queryByID", 1);
